...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.DisableAuthAutoRefresh != newCfg.DisableAuthAutoRefresh {
		changes = append(changes, fmt.Sprintf("disable-auth-auto-refresh: %t -> %t", oldCfg.DisableAuthAutoRefresh, newCfg.DisableAuthAutoRefresh))
	}
