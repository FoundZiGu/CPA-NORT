...
func authRequiresRefreshToken(a *Auth) bool {
	if a == nil {
		return false
	}
	switch strings.ToLower(strings.TrimSpace(a.Provider)) {
	case "antigravity", "claude", "codex", "kimi":
		return true
	default:
		return false
	}
...
func authHasRefreshToken(a *Auth) bool {
	...
	return strings.TrimSpace(metadataStringValue(a.Metadata, "refresh_token")) != ""
}

func metadataStringValue(meta map[string]any, key string) string {
	if meta == nil || key == "" {
		return ""
	}
