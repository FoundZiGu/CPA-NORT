...
func TestNextRefreshCheckAt_CodexWithoutRefreshTokenUnschedule(t *testing.T) {
	now := time.Date(2026, 4, 12, 0, 0, 0, 0, time.UTC)
	auth := &Auth{
		ID:       "a1",
		Provider: "codex",
...
func TestNextRefreshCheckAt_CodexWithRefreshTokenSchedules(t *testing.T) {
	now := time.Date(2026, 4, 12, 0, 0, 0, 0, time.UTC)
	auth := &Auth{
		ID:       "a1",
		Provider: "codex",
...
func TestAutoRefreshDisabledSkipsRefreshAndClearsSchedule(t *testing.T) {
	now := time.Date(2026, 4, 12, 0, 0, 0, 0, time.UTC)
	auth := &Auth{
		ID:       "a1",
		Provider: "test",
	...
	manager := NewManager(nil, nil, nil)
	manager.SetConfig(&internalconfig.Config{DisableAuthAutoRefresh: true})
	...
	if _, ok := loop.peek(); ok {
		t.Fatalf("loop.peek() ok = true, want false")
	}
}
