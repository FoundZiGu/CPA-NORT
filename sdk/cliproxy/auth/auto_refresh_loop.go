...
func (l *authAutoRefreshLoop) rebuild(now time.Time) {
	...
	if l.manager.authAutoRefreshDisabled() {
		l.mu.Lock()
		l.queue = l.queue[:0]
		l.index = make(map[string]*refreshHeapItem)
...
func (l *authAutoRefreshLoop) handleDueAuth(ctx context.Context, now time.Time, authID string) {
	...
	manager := l.manager
	if manager.authAutoRefreshDisabled() {
		l.remove(authID)
		return
	}
...
func nextRefreshCheckAt(now time.Time, auth *Auth, interval time.Duration) (time.Time, bool) {
	...
	if authRequiresRefreshToken(auth) && !authHasRefreshToken(auth) {
		return time.Time{}, false
	}
