...
// StopAutoRefreshLoop cancels only the background refresh loop.
func (m *Manager) StopAutoRefreshLoop() {
	m.mu.Lock()
...
func (m *Manager) rebuildAutoRefreshSchedule() {
	if m == nil {
		return
	}
	m.mu.RLock()
...
func (m *Manager) authAutoRefreshDisabled() bool {
	if m == nil {
		return false
	}
	cfg, _ := m.runtimeConfig.Load().(*internalconfig.Config)
	return cfg != nil && cfg.DisableAuthAutoRefresh
}
...
func (m *Manager) shouldRefresh(a *Auth, now time.Time) bool {
	...
	if authRequiresRefreshToken(a) && !authHasRefreshToken(a) {
		return false
	}
