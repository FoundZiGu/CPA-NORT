...
func (s *Service) applyConfigUpdate(newCfg *config.Config) {
	...
	var previousHomeEnabled bool
	var previousDisableAuthAutoRefresh bool
	s.cfgMu.RLock()
	if s.cfg != nil {
		previousStrategy = strings.ToLower(strings.TrimSpace(s.cfg.Routing.Strategy))
		previousSessionAffinity = s.cfg.Routing.SessionAffinity
		previousSessionAffinityTTL = s.cfg.Routing.SessionAffinityTTL
		previousHomeEnabled = s.cfg.Home.Enabled
		previousDisableAuthAutoRefresh = s.cfg.DisableAuthAutoRefresh
	}
	s.cfgMu.RUnlock()
	...
	if s.coreManager != nil {
		...
		switch {
		case newCfg.Home.Enabled || newCfg.DisableAuthAutoRefresh:
			if !previousHomeEnabled && !previousDisableAuthAutoRefresh {
				s.coreManager.StopAutoRefreshLoop()
				log.Info("core auth auto-refresh stopped")
			}
		case previousHomeEnabled || previousDisableAuthAutoRefresh:
